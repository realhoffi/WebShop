...
ausgabenmanager.directive('focusOnLoad', function ($timeout) {
	return function (scope, element, attrs) {
		$timeout(function () {
			element.focus();
		}, 500);
...
ausgabenmanager.directive('focusOnEnter', function ($log, $timeout) {
	return function (scope, element, attrs) {
		element.keypress(function (event) {
			var keycode = (event.keyCode ? event.keyCode : event.which);
			if (keycode == '13') {
				if (!attrs["focusOnEnter"]) {
					$log.error("Attribute not defined: focusOnEnter");
					return;
				} else {
					$log.info("Click to Element: " + attrs["focusOnEnter"]);
				}
				//alert('You pressed a "enter" key in somewhere' + attr);
